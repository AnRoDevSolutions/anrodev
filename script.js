...
(function initHeroCanvas() {
  ...
  const RIBBONS = [
    { yBase:0.50, lines:18, spread:0.34,
      amp1:0.20, amp2:0.09, freq1:1.55, freq2:2.9,
      phase1:0.0, phase2:1.1, speed:0.0022,
      color:[255,255,255], peak:0.48 },
    { yBase:0.44, lines:38, spread:0.24,
      amp1:0.17, amp2:0.08, freq1:1.85, freq2:2.6,
      phase1:2.2, phase2:0.5, speed:0.0017,
      color:[255,60,70], peak:0.38 },
    { yBase:0.56, lines:30, spread:0.20,
      amp1:0.15, amp2:0.07, freq1:2.0,  freq2:1.8,
      phase1:1.5, phase2:3.5, speed:0.0015,
      color:[255,200,40], peak:0.22 },
    { yBase:0.63, lines:34, spread:0.22,
      amp1:0.14, amp2:0.06, freq1:2.2,  freq2:1.5,
      phase1:4.1, phase2:3.0, speed:0.0018,
      color:[220,235,255], peak:0.36 },
  ];
  ...
  function draw() {
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        ctx.strokeStyle = `rgba(60,80,120,${alpha})`; ctx.lineWidth = 0.6; ctx.stroke();
      }
    }
    nodes.forEach(n => {
      ctx.beginPath(); ctx.arc(n.x, n.y, 2.2, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(60,80,130,0.50)'; ctx.fill();
    });
    animId = requestAnimationFrame(draw);
  }
  window.addEventListener('resize', () => { cancelAnimationFrame(animId); resize(); draw(); });
...
// ════════════════════════════════════════════════════════
//   DARK WAVE RIBBON FACTORY — Services, Why Us, Products
// ════════════════════════════════════════════════════════
function createWaveCanvas(canvasId, opts) {
  ...
  function drawRibbon(rb) {
    ...
    for (let li = 0; li < rb.lines; li++) {
      const frac = li/(rb.lines-1), dist = Math.abs(frac-0.5)*2;
      const bright = Math.pow(1-dist, 2.2), alpha = bright * rb.peak;
      if (alpha < 0.008) continue;
      const yOff = (frac-0.5)*spread, lp = li*0.13, spd = t*rb.speed;
      ctx.beginPath(); ctx.strokeStyle = `rgba(${r},${g},${b},${alpha})`; ctx.lineWidth = 0.4+bright*0.45;
      for (let x = 0; x <= W; x += 4) {
        const nx = x/W;
        const y = rb.yBase*H + yOff + Math.sin(nx*Math.PI*2*rb.freq1+spd+rb.phase1+lp)*amp1 + Math.sin(nx*Math.PI*2*rb.freq2-spd*0.65+rb.phase2+lp*0.5)*amp2;
        x===0 ? ctx.moveTo(x,y) : ctx.lineTo(x,y);
      }
      ctx.stroke();
    }
  }
  function draw() { ctx.clearRect(0,0,W,H); t+=1; RIBBONS.forEach(rb => drawRibbon(rb)); animId = requestAnimationFrame(draw); }
  window.addEventListener('resize', () => { cancelAnimationFrame(animId); resize(); draw(); });
  resize(); draw();
}
